...
public final class PoetryQueryHandler implements Handler<RoutingContext> {
  ...
  public void handle(RoutingContext routingContext) {
    ...
    routingContext
        .vertx()
        .executeBlocking(
            (Promise<List<GeneralChinesePoetry>> promise) -> {
              try {
                ...
                promise.complete(poetryList);
              } catch (Throwable e) {
                log.error("Query lucene throws exception:", e);
                promise.fail(e);
              }
            },
            false)
        .onSuccess(poetryList -> endReq(response, new SearchResult(0, "ok", poetryList)))
        .onFailure(t -> endReq(response, new SearchResult(t.getMessage())));
  }
